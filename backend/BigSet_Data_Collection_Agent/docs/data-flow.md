...
Routing: `extract_now` → direct extract; `requires_*` → Tinyfish Agent (if enabled); others → skip.

---

## Stage 5 — Extraction

### LLM output (row + sparse evidence + confidence)

The extraction agents return `row`, optional `evidence` (not required for every column), and `extraction_confidence`. They do **not** return `source_urls`.
