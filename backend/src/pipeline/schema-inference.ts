...
2. Pick primary key column(s) — use compound PKs when possible: one human-readable entity name column AND one guaranteed-unique column (such as a URL or ID), both marked as primary key. Make sure that each can independently identify the entity (not a composite AND-logic key — e.g. do not combine "Company Name" + "Plan Name" as a compound PK when neither alone is globally unique). Set \`is_primary_key: true\` on each primary key column. Set \`primary_key\` to the column name if there is one, or an array of column names if there are multiple. Every primary key column must have \`nullable: false\` and \`is_enumerable: true\`.
